...
- Reading uses one centred column. `CourseSidebar.astro` opens chapter navigation over the page; notes never resize or move the article. `annotation-marks.ts` draws Rough Notation outside `[data-annotatable]` from resolved ranges, preserving text anchors. `Bracket.astro` is for explicit author emphasis. Keep link underlines conventional, saved fonts intact, and Projects out of navigation and search while hidden.

- Use the same header on home and reading pages. `header.ts` owns mobile scroll visibility and must reveal focused controls. `SectionProgress.astro` and `section-progress.ts` own introduction and H2 progress links. Keep Chat and Brain rot directly visible. Merge adjacent annotation rectangles per visual line across syntax tokens without changing text anchors.
