...
import endereco from '../../ac/endereco-cache.typ?raw';
import hierarquia from '../../ac/hierarquia.dot?raw';
...
<Dot
  source={hierarquia}
  alt="Hierarquia da memória: o CPU acede às caches L1 de instruções e de dados em cerca de 1 ciclo, à L2 em cerca de 10, à RAM em cerca de 100 e ao disco em milissegundos."
/>
...
<Typst
  source={endereco}
  format="svg"
  alt="Endereço de 32 bits repartido em etiqueta de 21 bits (31 a 11), índice de 5 bits (10 a 6) e offset de 6 bits (5 a 0)."
/>
...
Quando um bloco novo precisa de um lugar ocupado, a **política de substituição** escolhe a vítima. As opções habituais são a **aleatória** (escolhe ao acaso, barata e surpreendentemente decente), a **FIFO** (expulsa o bloco há mais tempo na cache) e a **LRU** (_least recently used_, expulsa o bloco há mais tempo sem ser acedido). A LRU pura é cara de implementar, por isso as caches reais usam aproximações com um ou dois bits por bloco. A política só importa nas faltas de conflito e de capacidade: nas compulsórias, qualquer vítima serve porque o bloco pedido nunca esteve lá.
...
## Porque a memória principal é tão lenta: a DRAM

A penalidade de 100 ciclos do exemplo abaixo não é um número mágico: vem da anatomia da **DRAM**, a tecnologia da memória principal. Cada bit vive num condensador minúsculo que se descarrega sozinho e precisa de ser refrescado milhares de vezes por segundo. Os bits organizam-se em **linhas** (_rows_): para ler, o controlador abre uma linha inteira para uma fila de amplificadores (_row buffer_), lê as colunas pedidas em **rajada** (_burst_) e fecha a linha para a próxima.

Abrir uma linha custa dezenas de ciclos, e se a linha aberta não for a pedida paga-se fechar e abrir de novo. É por isso que acessos seguidos à mesma linha são rápidos e acessos saltados são lentos. Os chips dividem-se ainda em **bancos** independentes, para que uma leitura num banco se sobreponha à abertura de linha noutro. A cache esconde quase tudo isto; os 100 ciclos são o preço dos acessos que escapam.
...
Confirma a conta e estende-a a dois níveis, com uma L2 de 10 ciclos de hit e 20% de faltas para a memória de 100 ciclos:

<CodePlayground
  language="python"
  title="AMAT em cascata"
  code={'hit_l1, falta_l1 = 1, 0.05\nhit_l2, falta_l2, mem = 10, 0.2, 100\nprint(1 + falta_l1 * mem)\namat_l2 = hit_l2 + falta_l2 * mem\nprint(amat_l2)\nprint(hit_l1 + falta_l1 * amat_l2)'}
/>

O programa imprime `6.0`, depois `30.0` (o AMAT da L2) e por fim `2.5`: a L2 corta o custo médio de 6 para 2,5 ciclos.
...
<YouTube
  id="SAk-6gVkio0"
  title="Como a memória e as caches do CPU funcionam, Computerphile"
/>
...
- [Simulador interativo de cache](https://eecs370.github.io/simulators/cache/): experimenta índice, etiqueta e faltas com os teus próprios acessos.
