...
import contador from '../../ac/contador.dot?raw';

Os saltos condicionais são frequentes, cerca de uma em cada seis instruções nos programas típicos, e cada salto mal apostado no [pipeline](pipeline/) deita fora o trabalho das fases seguintes.[^saltos] A predição de saltos é o mecanismo que adivinha o resultado de cada salto antes de ele ser decidido, para o pipeline nunca parar. Os bons preditores modernos acertam mais de 95% das vezes.

[^saltos]: A frequência de saltos e as taxas de acerto típicas seguem o _Computer Organization and Design, versão RISC-V_, de Patterson e Hennessy, livro de referência da cadeira.
...
$$
\text{stalls de controlo} = \text{frequ\^encia de saltos} \times \text{taxa de erro} \times \text{penalidade}
$$
...
<Dot
  source={contador}
  alt="Máquina de quatro estados do contador saturante: T avança para a direita até 11 e N recua para a esquerda até 00, com saturação nos extremos."
/>
...
Corre os dois preditores sobre a sequência T, T, T, N, T e compara os acertos:

<CodePlayground
  language="python"
  title="Preditores de 1 e 2 bits"
  code={
    'def simula(seq, estado, bits):\n    for r in seq:\n        aposta = "T" if (estado == 1 if bits == 1 else estado >= 2) else "N"\n        print(r, "aposta", aposta, "acerto" if aposta == r else "erro")\n        if bits == 1:\n            estado = 1 if r == "T" else 0\n        else:\n            estado = min(3, estado + 1) if r == "T" else max(0, estado - 1)\n\nseq = ["T", "T", "T", "N", "T"]\nprint("1 bit, comeca em 1:")\nsimula(seq, 1, 1)\nprint("2 bits, comeca em 11:")\nsimula(seq, 3, 2)'
  }
/>

O de 1 bit acerta 3 em 5; o de 2 bits acerta 4 em 5, falhando só na saída do ciclo.
...
<YouTube
  id="nczJ58WvtYo"
  title="Como funciona a predição de saltos nos CPUs, Computerphile"
/>
...
- [Medições reais de predição de saltos](https://danluu.com/branch-prediction/): dados de desempenho em CPUs modernas, para veres os 95% a acontecer.
