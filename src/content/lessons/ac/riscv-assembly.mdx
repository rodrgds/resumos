...
import pilha from '../../ac/pilha.typ?raw';
...
<YouTube
  id="7tM4XkqoFro"
  title="Introdução à arquitetura RISC-V, DDCA capítulo 6 parte 1"
/>
...
Para bytes há `lb` e `sb`, com a variante `lbu` para bytes sem sinal:

```riscv
lb   t0, 0(t1)     # t0 = byte com sinal em memória[t1], estendido a 32 bits
lbu  t0, 0(t1)     # t0 = byte sem sinal (0 a 255)
sb   t0, 1(t1)     # guarda só o byte baixo de t0 em memória[t1 + 1]
```

Usa `lb` quando o byte representa um número com sinal (por exemplo um `char` em C) e `lbu` quando representa um valor de 0 a 255. O `sb` escreve um único byte sem mexer nos vizinhos, o que é essencial para cadeias de caracteres e estruturas compactas.
...
Corre a mesma soma com o vetor `[5, 7]` definido na secção de dados. O programa imprime `12` com uma chamada de sistema e termina:

<CodePlayground
  language="riscv"
  title="Soma de um vetor curto"
  code={`.text
.globl main
main:
    la a0, vetor
...
O editor corre RISC-V de 32 bits com as chamadas de sistema do RARS: `ecall` com `a7 = 1` imprime o inteiro em `a0` e com `a7 = 10` termina o programa. No qtrvsim, as chamadas de sistema seguem a mesma convenção do SPIM para estes dois casos: `1` imprime inteiro, `10` termina.
...
## Correr nos simuladores

O playground acima chega para exemplos curtos. Para os trabalhos, usa um simulador completo, que mostra registos e memória a cada instrução:

<Tabs id="simuladores-riscv" labels={['qtrvsim', 'RARS']}>
  <div slot="panel-0">

Guarda o programa num ficheiro `.s`, abre-o no qtrvsim e corre instrução a instrução. Observa `ra` antes e depois de cada `jal` e confirma que `sp` só mexe dentro das funções que reservam pilha.

  </div>
  <div slot="panel-1">

Abre o RARS, monta o mesmo ficheiro e corre instrução a instrução. As chamadas de sistema `1` e `10` comportam-se como no exemplo acima, por isso o que imprimires num lado imprime no outro.
...
O diagrama mostra a pilha antes e depois da reserva: `sp` desce 16 bytes e a zona nova guarda `ra` e `s0`.

<Typst
  source={pilha}
  format="svg"
  alt="Pilha antes e depois de reservar 16 bytes: sp desce e a zona nova guarda ra em 8(sp) e s0 em 0(sp)."
/>

Corre este padrão no qtrvsim ou no RARS com uma função que chama outra e observa `ra` e `sp` a cada passo. Ver o endereço de regresso a ser guardado e reposto tira a magia toda ao mecanismo.

## Para saber mais

- [qtrvsim](https://github.com/cvut/qtrvsim): o simulador usado nos trabalhos, com código fonte e manuais.
- [RARS](https://github.com/TheThirdOne/rars): simulador em Java para montar e correr RISC-V com chamadas de sistema.
- [Especificações oficiais do RISC-V](https://riscv.org/technical/specifications/): os volumes ratificados, incluindo o conjunto base não privilegiado.
