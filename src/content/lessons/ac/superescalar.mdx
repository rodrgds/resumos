...
import janela from '../../ac/superescalar-janela.typ?raw';
import dependencias from '../../ac/dependencias.dot?raw';
...
<Dot
  source={dependencias}
  alt="Quatro instruções com as três dependências: RAW a cheio de 1 para 2 e de 3 para 4 em t0, WAR a tracejado de 2 para 3 em t3 e WAW a tracejado de 1 para 4 em t0."
/>
...
<Typst
  source={janela}
  format="svg"
  alt="Emissão em duas vias: a instrução 2 bloqueada ocupa a via B enquanto a instrução 3, independente, passa à frente pela via livre."
/>
...
Segue três escritas em `t0` a receberem físicos distintos, com `p` para os registos físicos:

| Instrução        | Lê físicos   | Escreve físico novo |
| ---------------- | ------------ | ------------------- |
| `add t0, t1, t2` | `t1`, `t2`   | `p40` para `t0`     |
| `sub t3, t0, t4` | `p40` (`t0`) | `p42` para `t3`     |
| `add t0, t5, t6` | `t5`, `t6`   | `p41` para `t0`     |

A segunda escrita em `t0` recebe `p41` em vez de reutilizar `p40`: a WAW entre a primeira e a terceira instrução desaparece, e um leitor atrasado de `p40` continua a ver o valor certo. É assim que nomes diferentes eliminam as dependências falsas sem mudar o programa.
...
Junta as peças e vês o teto: a largura de emissão, o tamanho da janela, a precisão da [predição de saltos](predicao-saltos/) (cada erro deita fora a janela inteira) e, acima de tudo, o paralelismo real do programa. Como regra prática, passar de 2 para 4 vias duplica o hardware de emissão e de forwarding, mas o IPC típico sobe muito menos, porque os programas comuns têm paralelismo limitado.[^vias] É este rendimento decrescente, somado ao muro da energia, que empurrou a indústria para vários núcleos em vez de núcleos cada vez mais largos. E isso é a próxima página.

[^vias]: A quantificação exata depende do programa e da microarquitetura; o _Computer Organization and Design_ de Patterson e Hennessy, livro de referência da cadeira, mede estes rendimentos com benchmarks SPEC.
