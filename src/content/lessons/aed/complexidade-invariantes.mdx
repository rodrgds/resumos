...
import curvas from '../../aed/complexidade-curvas.typ?raw';
...
<Typst
  source={curvas}
  format="svg"
  alt="Gráfico das curvas O(1), O(log n), O(n), O(n log n) e O(n²) para n de 1 a 10. A curva quadrática sobe muito mais depressa que as outras."
/>

Repara como a quadrática se destaca das outras mesmo com $n$ pequeno: é por isso que um algoritmo $O(n^2)$ aguenta os testes pequenos do Mooshak e rebenta no limite de tempo dos grandes.
...
O preço da binária está fora da pesquisa: alguém teve de ordenar o vetor primeiro, e ordenar custa pelo menos $O(n \log n)$[^limite]. Se pesquisares uma vez num vetor desordenado, ordenar mais pesquisar sai mais caro que a linear. A análise decide consoante o uso, não em abstrato.

<CodePlayground
  language="cpp"
  title="Comparações: linear contra binária"
  code={
    '#include <iostream>\n#include <vector>\nusing namespace std;\n\nint main() {\n    vector<int> v(1000);\n    for (int i = 0; i < 1000; i++) v[i] = i;\n    int alvo = 2000, linear = 0, binaria = 0;\n    for (int x : v) { linear++; if (x == alvo) break; }\n    int lo = 0, hi = 999;\n    while (lo <= hi) {\n        binaria++;\n        int meio = (lo + hi) / 2;\n        if (v[meio] == alvo) break;\n        if (v[meio] < alvo) lo = meio + 1; else hi = meio - 1;\n    }\n    cout << "linear: " << linear << "\\nbinaria: " << binaria << "\\n";\n}'
  }
/>

O programa imprime `linear: 1000` e `binaria: 10`: o alvo 2000 não existe no vetor, por isso a linear percorre tudo e a binária parte o intervalo dez vezes até o esvaziar. Muda o alvo para 499 e vê as duas contagens caírem.
...
<YouTube
  id="jZzyERW7h1A"
  title="CS50x 2024, aula 3: algoritmos e análise de custo"
/>

Para rever o argumento das comparações com outra voz, a aula 3 do CS50x constrói a pesquisa binária e a notação assintótica a partir do zero.

[^limite]: O limite $\Omega(n \log n)$ vale para ordenações que só comparam pares de elementos: com $n!$ ordens possíveis e dois resultados por comparação, são precisas pelo menos $\log_2(n!) \approx n \log n$ comparações. Segues a conta completa na [página de ordenação](pesquisa-ordenacao/).
