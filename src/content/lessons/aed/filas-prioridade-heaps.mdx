...
import heap from '../../aed/heap-arvore-vetor.typ?raw';
...
<Typst
  source={heap}
  format="svg"
  alt="O heap final desenhado como árvore e como vetor: raiz 1, filhos 3 e 4, e por baixo o vetor 1, 3, 4, 7, 5, 9 com os índices 0 a 5."
/>
...
<CodePlayground
  language="cpp"
  title="Inserções com sift-up"
  code={
    '#include <iostream>\n#include <vector>\nusing namespace std;\nint main() {\n    vector<int> h;\n    for (int x : {7, 3, 9, 1, 5, 4}) {\n        h.push_back(x);\n        int i = h.size() - 1;\n        while (i > 0 && h[(i - 1) / 2] > h[i]) {\n            swap(h[i], h[(i - 1) / 2]);\n            i = (i - 1) / 2;\n        }\n        for (int v : h) cout << v << " ";\n        cout << "\\n";\n    }\n}'
  }
/>

O programa imprime uma linha por inserção: `7`, `3 7`, `3 7 9`, `1 3 9 7`, `1 3 9 7 5` e `1 3 4 7 5 9`, exatamente os vetores que seguiste acima. Repara na quarta linha: o 1 sobe duas posições, uma por cada comparação com um pai maior.
...
O [Visualgo](https://visualgo.net/en/heap) mostra o sift-up e o sift-down lado a lado com o vetor: corre as mesmas seis inserções e vê cada troca acontecer na árvore e no vetor ao mesmo tempo.[^linear]

[^linear]: A construção linear soma o trabalho por nível: cerca de $n/2$ nós que descem no máximo 1, $n/4$ que descem no máximo 2, e por aí fora. A série $\sum_i n \cdot i / 2^{i+1}$ converge para $O(n)$, porque os níveis caros têm pouquíssimos nós.
