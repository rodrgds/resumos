...
import eixos from '../../alga/proprios-eixos.typ?raw';
...
Os dois vetores próprios definem os eixos naturais da matriz: ao longo de cada um, $A$ só estica.

<Typst
  source={eixos}
  format="svg"
  alt="Duas retas pela origem nas direções (1, 1) e (1, menos 2), os eixos próprios da matriz."
/>
...
<Tabs id="diagonalizar-ou-nao" labels={['Diagonaliza', 'Não diagonaliza']}>
  <div slot="panel-0">

<p>
  A matriz do exemplo tem valores próprios 5 e 2, distintos: há base própria e P
  menos 1 A P é diagonal.
</p>
...
<p>
  A rotação de 90 graus tem polinómio característico lambda ao quadrado mais 1,
  sem raízes reais: sobre R não há vetores próprios e não há diagonalização.
</p>
...
O programa seguinte calcula os valores próprios da matriz $A$ pela fórmula resolvente do polinómio característico. Corre-o e confirma que obténs `2.0` e `5.0`; depois troca os quatro valores e testa as tuas matrizes.

<CodePlayground
  language="python"
  title="Valores próprios 2x2"
  code={
    'a, b, c, d = 4, 1, 2, 3\nt = a + d\ndet = a * d - b * c\ndisc = t * t - 4 * det\nr = disc ** 0.5\nprint((t - r) / 2, (t + r) / 2)'
  }
/>
...
Este vídeo mostra vetores próprios como os eixos que a matriz só estica.
...
- [Vetores e valores próprios](https://www.3blue1brown.com/lessons/eigenvalues/), a versão animada dos eixos desta página.
