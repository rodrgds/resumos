...
import taylor from './taylor-exp.typ?raw';
...
<Typst
  source={taylor}
  format="svg"
  alt="Gráfico da exponencial com os polinómios de Taylor de graus 1, 2 e 3 em torno de zero. Cada grau novo cola-se à curva num intervalo maior."
/>

Cada grau cola-se à curva num intervalo maior: $P_1$ só acerta junto a $0$, $P_3$ acompanha a exponencial bem para além de $x = 1$. Confirma os números de $P_3(0{,}1)$ correndo o cálculo:

<CodePlayground
  language="javascript"
  title="P_3(0,1) contra a exponencial"
  code={
    'const x = 0.1;\nconst p3 = 1 + x + (x * x) / 2 + (x * x * x) / 6;\nconsole.log(p3.toFixed(6));\nconsole.log(Math.exp(x).toFixed(6));\nconsole.log(Math.abs(Math.exp(x) - p3).toExponential(1));'
  }
/>

O programa imprime `1.105167`, `1.105171` e `4.3e-6`: o erro de $P_3$ em $0{,}1$ é de poucas milionésimas.
...
para algum $z$ entre $a$ e $x$. Não sabemos $z$, mas se conseguirmos majorar $|f^{(n+1)}|$ no intervalo, obtemos uma garantia de erro.[^resto]
...
<YouTube id="3d6DsjIBzJ4" title="Taylor series (3Blue1Brown)" />
...
- [Taylor series, artigo interativo](https://www.3blue1brown.com/lessons/taylor-series/), que acompanha o vídeo com aproximações de `cos(x)` que podes manipular.

[^resto]: A forma do resto de Lagrange e as condições para passar da fórmula à série seguem o capítulo de aproximação polinomial de _Análise Matemática 1, Conteúdo teórico e aplicações_ (Carlos A. Conceição António, AEFEUP, 2017), a bibliografia indicada da cadeira.
