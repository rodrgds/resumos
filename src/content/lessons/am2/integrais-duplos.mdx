...
import regiao from '../../am2/regiao-dupla-ordem.typ?raw';
import setor from '../../am2/setor-polar.typ?raw';
...
<Typst
  source={regiao}
  format="svg"
  alt="Região triangular entre y igual a x e y igual a 1, com uma seta vertical para a ordem dy dx e uma seta horizontal para a ordem dx dy."
/>

## Trocar a ordem quando a primitiva bloqueia

<Tabs id="ordens-integracao" labels={['Na ordem dada', 'Trocando a ordem']}>
  <div slot="panel-0">

  A região entre $y = x$ e $y = 1$ com $x \in [0, 1]$ escreve-se como $\int_0^1 \int_x^1 e^{y^2} \, dy \, dx$, mas $e^{y^2}$ não tem primitiva elementar em $y$: por esta ordem, o integral está bloqueado.

  </div>
  <div slot="panel-1">

  A região é a mesma descrita ao contrário: para cada $y \in [0, 1]$, $x$ vai de $0$ a $y$.
...
<Typst
  source={setor}
  format="svg"
  alt="Setor polar com um pedacinho de área destacado: o arco cresce com r, e cada pedacinho mede r dr d theta."
/>
...
<YouTube id="HA41kYxVYnw" title="Double Integrals over Polar Regions (Professor Leonard)" />

- O manual aberto [Calculus Volume 3 (OpenStax)](https://openstax.org/details/books/calculus-volume-3), com capítulos de integrais múltiplos e exercícios resolvidos.
