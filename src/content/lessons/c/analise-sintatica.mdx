...
Um analisador descendente escolhe a regra olhando só para o próximo símbolo. Com a gramática sem recursão à esquerda $E \to T R$, $R \to + T R \mid \epsilon$, $T \to \text{id}$, os conjuntos de previsão são $\text{FIRST}(E) = \text{FIRST}(T) = \{\text{id}\}$, $\text{FIRST}(R) = \{+, \epsilon\}$ e $\text{FOLLOW}(R) = \{\text{fim}\}$, onde `fim` representa o marcador `$`. A tabela de decisão fica assim: em $E$ com `id` aplica $E \to T R$; em $R$ com `+` aplica $R \to + T R$; em $R$ com `$` aplica $R \to \epsilon$.
