...
import triedro from '../../f1/triedro.typ?raw';
...
<Typst
  source={triedro}
  format="svg"
  alt="Ponto P numa curva com centro de curvatura C. A velocidade é tangente à curva e a aceleração centrípeta aponta de P para C ao longo do raio rho."
/>

O desenho resume a geometria: a velocidade segue a tangente e a componente normal aponta sempre para o centro de curvatura, com módulo $v^2/\rho$.
...
<CodePlayground
  language="python"
  title="Integrar o atrito viscoso por Euler"
  code={
    'v, x, dt = 10.0, 0.0, 0.1\nfor _ in range(40):\n    v *= 1 - 0.25 * dt\n    x += v * dt\nprint(f"v(4 s) = {v:.2f} m/s, x(4 s) = {x:.2f} m")'
  }
/>

O programa integra a mesma equação do exemplo com o método de Euler e imprime `v(4 s) = 3.63 m/s, x(4 s) = 24.83 m`. Compara com os valores analíticos $3{,}7\ \text{m/s}$ e $25{,}3\ \text{m}$: o método numérico fica ligeiramente abaixo porque usa a velocidade do início de cada passo. Experimenta reduzir `dt` para `0.01` (e aumentar as iterações para 400) e vê os valores aproximarem-se.
