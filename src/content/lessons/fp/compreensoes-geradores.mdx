...
import consumo from '../../fp/gerador-consumo.typ?raw';
...
O esquema mostra o consumo único: a primeira travessia entrega valores, a segunda já vem vazia.

<Typst
  source={consumo}
  format="svg"
  alt="Diagrama do consumo único de um gerador: a primeira travessia entrega os valores, a segunda vem vazia."
/>
...
Corre com a lista do exemplo e confirma a ordenação.

<CodePlayground
  language="python"
  title="Quicksort com compreensões"
  code={
    'def quicksort(valores):\n    if len(valores) <= 1:\n        return valores\n    pivo = valores[0]\n    menores = [x for x in valores[1:] if x <= pivo]\n    maiores = [x for x in valores[1:] if x > pivo]\n    return quicksort(menores) + [pivo] + quicksort(maiores)\n\nprint(quicksort([3, 6, 1, 5, 2, 4]))'
  }
/>

A mesma ideia com lista materializada e com gerador, para veres a diferença de memória:

<Tabs id="quicksort-memoria" labels={['Lista', 'Gerador']}>
  <div slot="panel-0">
...
Guarda todos os menores de uma vez.
...
Produz um de cada vez, sem guardar a lista.
