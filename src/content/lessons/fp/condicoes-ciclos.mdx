...
O grafo mostra como o programa escolhe: testa por ordem e segue o primeiro ramo verdadeiro.

<Dot
  source={
    'digraph { rankdir=TB; nota [shape=box]; g18 [label=">= 18?"]; g14 [label=">= 14?"]; g10 [label=">= 10?"]; nota -> g18; g18 -> Excelente [label=sim]; g18 -> g14 [label=não]; g14 -> Bom [label=sim]; g14 -> g10 [label=não]; g10 -> Suficiente [label=sim]; g10 -> Reprovado [label=não]; }'
  }
  alt="Grafo de decisão: a nota é testada por ordem, 18, depois 14, depois 10, e segue o primeiro ramo verdadeiro."
/>
...
O mesmo cálculo com `while` dá o mesmo resultado; só muda a forma de avançar. Corre as duas versões e compara.

<CodePlayground
  language="python"
  title="Soma de 1 a 100 com for e com while"
  code={
    'soma_for = 0\nfor i in range(1, 101):\n    soma_for += i\nprint(soma_for)\n\nsoma_while = 0\nn = 1\nwhile n <= 100:\n    soma_while += n\n    n += 1\nprint(soma_while)'
  }
/>
