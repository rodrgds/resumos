...
import fluxo from '../../fp/segundos-fluxo.typ?raw';
...
O diagrama resume o caminho dos dados: o total origina as horas e um resto, e o resto origina os minutos e os segundos.

<Typst
  source={fluxo}
  format="svg"
  alt="Diagrama do fluxo de dados: o total origina as horas por divisão inteira e um resto; o resto origina os minutos e os segundos."
/>
...
Um programa útil lê dados e escreve resultados. `input()` lê uma linha do teclado e devolve sempre uma string[^input-doc]; `print()` escreve valores no ecrã, separados por espaços.
...
Corre o programa com a entrada `3661` e confirma que escreve `1 1 1`.

<CodePlayground
  language="python"
  title="Segundos em horas, minutos e segundos"
  input="3661"
  code={
    'total = int(input())\nhoras = total // 3600\nresto = total % 3600\nminutos = resto // 60\nsegundos = resto % 60\nprint(horas, minutos, segundos)'
  }
...
[^input-doc]: Documentação oficial do Python, [função `input`](https://docs.python.org/3/library/functions.html#input).
