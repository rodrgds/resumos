...
O grafo mostra o pipeline: a lista entra, o filtro guarda as aprovações e a redução agrega.

<Dot
  source={
    'digraph { rankdir=LR; notas [shape=box]; filtrar [label="filter (>= 10)"]; agregar [label="reduce (+)"]; media [shape=box]; notas -> filtrar -> agregar -> media; }'
  }
  alt="Pipeline funcional: as notas passam pelo filtro das aprovações e depois pela agregação da soma."
/>

Corre os dois casos, incluindo a lista sem aprovações.

<CodePlayground
  language="python"
  title="Média das aprovações"
  code={
    'from functools import reduce\n\ndef media_aprovados(notas):\n    aprovadas = list(filter(lambda n: n >= 10, notas))\n    if not aprovadas:\n        return 0\n    total = reduce(lambda a, b: a + b, aprovadas)\n    return total / len(aprovadas)\n\nprint(media_aprovados([8, 12, 15, 7, 10]))\nprint(media_aprovados([5, 7]))'
  }
/>

A mesma média em dois estilos, para comparares a legibilidade:

<Tabs id="media-estilos" labels={['Funcional', 'Compreensão']}>
  <div slot="panel-0">
...
```python
aprovadas = [n for n in notas if n >= 10]
