...
import alias from '../../fp/alias-copia.typ?raw';
...
Corre as duas versões e repara na estrutura do resultado.

<CodePlayground
  language="python"
  title="append contra extend"
  code={
    'a = [1, 4]\na.append([5, 1, 3])\nprint(a)\nb = [1, 4]\nb.extend([5, 1, 3])\nprint(b)'
  }
/>
...
O esquema contrasta os dois casos: no alias, dois nomes apontam para a mesma lista; na cópia, cada nome tem a sua.

<Typst
  source={alias}
  format="svg"
  alt="Diagrama do alias contra a cópia: os nomes notas e copia apontam para a mesma lista; o nome c aponta para uma lista separada com os mesmos elementos."
/>

A mesma diferença em código, lado a lado:

<Tabs id="alias-copia" labels={['Alias', 'Cópia']}>
  <div slot="panel-0">
...
Escreve `[12, 15, 18]`: a lista é partilhada.
...
Escreve `[12, 15]` e `[12, 15, 18]`: são independentes.
...
<YouTube
  id="6wvL4jH65Do"
  title="Listas, tuplos, dicionários e conjuntos no Python"
/>

## Para saber mais

- Tutorial oficial do Python sobre [estruturas de dados](https://docs.python.org/3/tutorial/datastructures.html): listas, tuplos, conjuntos, dicionários e compreensões.
