...
import bitsLinha from '../../fsc/bits-linha.typ?raw';
...
<Typst
  source={bitsLinha}
  format="svg"
  alt="Cinco bits 1 1 0 0 1 em fila, com as potências 2 à quarta até 2 à zero por baixo. O primeiro bit está marcado MSB e o último LSB."
/>

Repara no que o diagrama mostra: cada posição tem um peso fixo, e o valor do número é a soma dos pesos das posições a `1`. O MSB decide a ordem de grandeza; o LSB decide a paridade.

O código ASCII usa 7 bits para representar texto: cada letra, dígito e símbolo tem um número. A letra `A` maiúscula vale 65, a minúscula `a` vale 97 e o dígito `0` vale 48. É por isso que ordenar texto pelo código põe os dígitos antes das maiúsculas e estas antes das minúsculas. Um byte guarda um caráter ASCII com o oitavo bit a zero.
...
Por isso $0{,}75_{10} = 0{,}11_2$. Nem todas as frações decimais têm representação binária finita. A fração $0{,}692$, por exemplo, gera bits sem fim[^finitas]. Juntando as duas metades com um exemplo exato: $25{,}75_{10} = 11001{,}11_2$.

Podes confirmar estas conversões com um programa curto. Corre-o e compara as saídas com as contas feitas à mão:
...
A primeira linha escreve `0b11001`, onde o prefixo `0b` marca a base 2. A segunda faz o caminho inverso. A terceira confirma que $0{,}75 = 3/4$, ou seja, $(3, 4)$.
...
[^finitas]: Multiplicar por 2 e registar a parte inteira é o método geral para a parte fracionária. Com $0{,}692$ obténs $0{,}692 \times 2 = 1{,}384$, depois $0{,}384 \times 2 = 0{,}768$, depois $0{,}768 \times 2 = 1{,}536$, e os restos nunca chegam a zero, por isso os bits continuam sem fim.
