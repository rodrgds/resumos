...
import camadas from '../../fsi/camadas.dot?raw';
...
<Dot
  source={camadas}
  alt="Grafo das camadas de defesa em profundidade: o atacante atravessa a firewall, a autenticação, as permissões e a cifragem até chegar às cópias."
/>
...
O passo 1, em comandos, é curto e vale a pena correr na tua máquina virtual para sentires o que "fechado" significa:

```console
$ sudo useradd copias
$ sudo mkdir -p /srv/copias
$ sudo chown copias:copias /srv/copias
$ sudo chmod 700 /srv/copias
$ ls -l /srv
drwx------ 2 copias copias 4096 ... copias
```

O `700` dá tudo ao dono e nada a mais ninguém, e o `ls -l` confirma: só o utilizador `copias` entra na pasta. Qualquer outro utilizador, incluindo um serviço Web comprometido, esbarra aqui antes sequer de ver os arquivos.
