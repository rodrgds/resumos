...
import ciclo from '../../ia/ciclo-agente.typ?raw';
...
O ciclo completo do aspirador fica assim: as salas produzem uma perceção através dos sensores, o agente decide, e a ação altera as salas através dos atuadores.[^russell]

<Typst
  source={ciclo}
  format="svg"
  alt="Diagrama do ciclo perceção-ação no aspirador de duas salas: as salas geram a perceção (A, sujo) através dos sensores, o agente escolhe a ação Aspirar, e a ação limpa as salas através dos atuadores."
/>
...
<Tabs id="arquiteturas-aspirador" labels={['Reativo', 'Objetivos', 'Utilidade']}>
  <div slot="panel-0">
    O reativo limpa as duas salas com quatro regras e sem memória. Escolhe-o quando a perceção atual chega para decidir e cada ação é barata. No aspirador, vagueia depois de limpar porque nenhuma regra reconhece o trabalho feito.
  </div>
  <div slot="panel-1">
    O baseado em objetivos guarda o que já viu e para quando as duas salas estão limpas. Escolhe-o quando há um estado final claro e vale a pena pesquisar a sequência até lá. É a arquitetura das páginas de pesquisa.
  </div>
  <div slot="panel-2">
    O baseado em utilidade compara custos: 3 ações contra 40, bateria gasta contra bateria poupada. Escolhe-o quando há compromissos entre qualidade e custo, como aceitar 95 por cento limpo por metade da energia.
  </div>
</Tabs>
...
- O manual da cadeira dedica o capítulo 2 aos agentes e usa este mesmo aspirador de duas salas como exemplo de abertura.[^russell]
- [Página do manual Russell e Norvig](https://aima.cs.berkeley.edu/3rd-ed/index.html), com os capítulos por tema.

[^russell]: Russell, S., Norvig, P., Artificial Intelligence: A Modern Approach, cap. 2. Bibliografia obrigatória da ficha da cadeira.
