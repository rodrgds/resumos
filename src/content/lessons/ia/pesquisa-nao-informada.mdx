...
import puzzle from '../../ia/puzzle8.dot?raw';
...
<Dot
  source={puzzle}
  alt="Grafo do fragmento do 8-puzzle: S no topo liga a A, B, C e D; A liga a A1 e A2; B liga a B1 e B2. O caminho S, B, B2 está destacado e B2 está marcado como objetivo."
/>

A largura sobre este fragmento, executável abaixo, expande por níveis e gera B2 na expansão de B.[^berkeley]

<CodePlayground
  language="python"
  title="BFS no fragmento"
  code={
    'from collections import deque\n\nsucessores = {\n    "S": ["A", "B", "C", "D"],\n    "A": ["A1", "A2"],\n    "B": ["B1", "B2"],\n}\nvisitados = {"S"}\nfila = deque(["S"])\nwhile fila:\n    n = fila.popleft()\n    print(n)\n    for m in sucessores.get(n, []):\n        if m not in visitados:\n            visitados.add(m)\n            fila.append(m)'
  }
/>

Isto escreve `S A B C D A1 A2 B1 B2`, a ordem de expansão da largura: B2 aparece durante a expansão de B, a distância 2 do início.
...
[^berkeley]: O projeto Search de Berkeley CS188 implementa BFS, DFS, UCS e A\* com heurísticas admissíveis: https://berkeleyai.github.io/cs188-website/project1.html.
