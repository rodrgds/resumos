...
import ciclo from '../../ipc/ciclo-ihc.typ?raw';
...
Dizer "a app tem bom UI" quando queres dizer que é fácil de usar é o erro de vocabulário mais comum. Guarda a distinção: UI desenha-se, UX sente-se e mede-se. A bibliografia da cadeira define este vocabulário com mais detalhe.[^fcg]

<Typst
  source={ciclo}
  format="svg"
  alt="Diagrama do ciclo pessoa sistema. A pessoa age com toques e voz sobre o sistema, que responde com ecrãs e sons. A UI é a superfície de contacto e a UX é a experiência total."
/>

[^fcg]: Fonseca, M. J., Campos, P. e Gonçalves, D., Introdução ao Design de Interfaces, FCA, 2012. É a bibliografia obrigatória da ficha da unidade curricular.
...
<Tabs id="maus-exemplos" labels={['Multibanco', 'Formulário', 'Painel']}>
  <div slot="panel-0">

No multibanco, levantar dinheiro e consultar o saldo pedem PINs diferentes em ecrãs quase iguais. O utilizador decora dois códigos em vez de reconhecer uma opção. Falha de memória e de consistência.

  </div>
  <div slot="panel-1">

No formulário de candidatura, o campo do código postal rejeita o hífen sem dizer qual é o formato aceite. O candidato tenta três variações e desiste. Falha de prevenção de erros e de mensagens úteis.

  </div>
  <div slot="panel-2">

No painel de administração, "Guardar" e "Apagar" são botões gémeos lado a lado. Um clique apressado apaga dados sem confirmação. Falha de distinção visual e de proteção contra lapsos.
