...
<Tabs id="interfaces-compra" labels={['Antes: parede de campos', 'Depois: com hierarquia']}>
  <div slot="panel-0">

Nome, email, cartão, morada de faturação e três botões parecidos, tudo ao mesmo nível. O comprador preenche sem saber o que está a comprar nem quanto custa, e um erro no cartão aparece no topo, longe do campo.

  </div>
  <div slot="panel-1">

Título com a sessão e a data, resumo do pedido com o preço total destacado, e só depois o formulário com um rótulo por campo. O erro aparece junto ao campo, em texto, e o comprador confirma o pedido antes de dar dados.
...
O critério do WCAG para texto normal pede um rácio de contraste de pelo menos 4,5 para 1.[^wcag] Mede com estes dois casos sobre fundo branco:

- Preço em cinzento claro `#9CA3AF`: rácio 2,54 para 1. Chumba, e é por isso que o preço "elegante" se torna ilegível ao sol.
- O mesmo preço em `#595959`: rácio 7,00 para 1. Passa com margem.

Escurecer o texto até o rácio passar de 4,5 é a correção mais barata da página. Para texto grande (a partir de cerca de 24 px) e elementos de interface, o mínimo desce para 3 para 1, mas nada se perde em manter o 4,5 em todo o lado.

<YouTube
  id="Igf560KIFSo"
  title="Beyond the Basics Accessible Form Validation That Works"
/>

## Para saber mais

- [Labels nos formulários](https://www.w3.org/WAI/tutorials/forms/labels/), o tutorial oficial do W3C sobre ligar rótulos a campos.
- [Formulários acessíveis](https://web.dev/learn/accessibility/forms), o guia prático com HTML nativo.

[^wcag]: W3C, [Understanding Success Criterion 1.4.3: Contrast (Minimum)](https://www.w3.org/WAI/WCAG21/Understanding/contrast-minimum.html), o critério dos 4,5 para 1 para texto normal.
