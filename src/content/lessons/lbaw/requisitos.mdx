...
import requisitosFigura from '../../lbaw/requisitos.typ?raw';
...
<Typst
  source={requisitosFigura}
  format="svg"
  alt="Diagrama com os atores Visitante e Comprador à esquerda. O Visitante liga-se ao caso de uso Pesquisar eventos. O Comprador liga-se a Pesquisar eventos, Comprar bilhete e Consultar histórico."
/>

O diagrama resume o âmbito: dois atores, três casos de uso, e o Visitante só toca na pesquisa. Tudo o que não couber nestas setas fica fora do projeto ou vira um requisito novo com ator próprio.
...
<Tabs id="requisitos-caso-historia" labels={['Caso de uso', 'História']}>
  <div slot="panel-0">

O caso de uso 1 completo: o comprador escolhe a sessão, o sistema mostra preço e lugares, o comprador confirma, o sistema reserva e confirma. Inclui a exceção de esgotar entre a escolha e a confirmação.

  </div>
  <div slot="panel-1">

A história equivalente: "como comprador, quero receber confirmação da compra para ter prova do que paguei". O fluxo e a exceção continuam a existir, mas vivem nos critérios de aceitação da história em vez de num documento separado.
...
A tabela cabe num programa pequeno. Corre-o e confirma que imprime as quatro linhas acordadas:

<CodePlayground
  language="php"
  title="Tabela de requisitos em PHP"
  code={
    '$requisitos = [\n    ["R1", "Comprar bilhetes", "Comprador", "Alta"],\n    ["R2", "Registar conta", "Visitante", "Alta"],\n    ["R3", "Pesquisar eventos", "Visitante", "Média"],\n    ["R4", "Consultar histórico", "Comprador", "Média"],\n];\nforeach ($requisitos as [$id, $texto, $ator, $prioridade]) {\n    echo "$id | $texto | $ator | $prioridade\n";\n}'
  }
/>

A saída lista `R1` a `R4` com ator e prioridade. Se um requisito novo não couber neste formato de uma linha, provavelmente ainda está vago.
