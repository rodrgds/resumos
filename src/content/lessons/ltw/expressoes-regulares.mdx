...
import anatomia from '../../ltw/anatomia-regex.typ?raw';
...
<YouTube id="rhzKDrUiJVk" title="Learn Regular Expressions In 20 Minutes" />

## As peças básicas

Uma expressão lê-se da esquerda para a direita, com cada peça a casar com um pedaço do texto. Lê o diagrama da esquerda para a direita: cada caixa é uma peça, cada legenda diz o que ela exige.

<Typst
  source={anatomia}
  format="svg"
  alt="Anatomia da expressão de telemóvel: circunflexo ancora o início, 9 exige o dígito 9, barra-d oito vezes exige oito dígitos, dólar ancora o fim."
/>
...
O formulário de registo da mercearia pede email, data de nascimento e telemóvel. Corre o testador com o caso bom, o caso mau e o caso manhoso, e confirma que só o primeiro passa:

<CodePlayground
  language="javascript"
  code={
    'const tel = /^9\\d{8}$/;\nconsole.log(tel.test("912345678"));\nconsole.log(tel.test("812345678"));\nconsole.log(tel.test("xx912345678xx"));'
  }
/>

A saída é `true`, `false`, `false`: o número certo passa, o que começa por 8 falha no literal e o texto à volta falha nas âncoras. A mesma regra vive nas três frentes; compara-as:

<Tabs id="tres-frentes" labels={['HTML', 'PHP', 'JavaScript']}>
  <div slot="panel-0">

No HTML, o atributo `pattern` valida antes de enviar:
...
- Testador interativo de expressões regulares, com explicação peça a peça: https://regex101.com/.
