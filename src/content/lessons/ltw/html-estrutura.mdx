...
import arvore from '../../ltw/arvore-documento.typ?raw';

O HTML descreve a estrutura da página: o que é um título, o que é um parágrafo, onde começa um formulário. O navegador usa essa estrutura para mostrar a página, e os leitores de ecrã e os motores de pesquisa usam-na para a entender[^mdn-formularios]. Escrever bom HTML é escolher a etiqueta certa para cada pedaço de conteúdo, não empilhar `div` genéricos.

[^mdn-formularios]: MDN, guia de formulários web, com a regra de ligar cada `label` ao campo e as práticas de validação usadas nesta página: https://developer.mozilla.org/en-US/docs/Learn_web_development/Extensions/Forms.
...
A mesma página vista como árvore, que é como o navegador e o JavaScript a veem:

<Typst
  source={arvore}
  format="svg"
  alt="Árvore do documento mínimo: html divide-se em head e body; body divide-se em header e main; main contém article."
/>
...
<YouTube id="ok-plXXHlWw" title="HTML in 100 Seconds" />
...
Experimenta o formulário: prime **Encomendar** sem preencher o nome e o navegador queixa-se; escreve zero na quantidade e ele recusa. A moldura bloqueia o envio de propósito, por isso nada chega a lado nenhum, mas a validação é a real.
...
- Guia de formulários da MDN, com estrutura, controlos, validação e envio: https://developer.mozilla.org/en-US/docs/Learn_web_development/Extensions/Forms.
