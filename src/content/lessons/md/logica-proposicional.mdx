...
import tarski from './tarski.typ?raw';
...
Isto chama-se **condicional material**: ele mede valores de verdade, não causas.[^condicional] "Se $2 + 2 = 5$ então eu sou o rei de França" é uma frase verdadeira em lógica, porque o antecedente é falso. Parece estranho, mas é o que permite tratar "se... então" com tabelas. Na página sobre [provas com condicionais](/cadeiras/md/provas-proposicionais/) vais ver como traduzir expressões como "só se", "se", "a menos que" e "sempre que", que é onde quase toda a gente erra.
...
Repara que o resultado final é V só na linha $P = F$, $Q = V$. Ou seja, a fórmula equivale a $\lnot P \land Q$. Confirma com o avaliador, que imprime a coluna final em V e F:

<CodePlayground
  language="python"
  code={
    'for P in (True, False):\n    for Q in (True, False):\n        r = (P or Q) and not P\n        print("V" if r else "F")  # F F V F'
  }
/>

Este processo mecânico serve para verificar equivalências quando tens poucas variáveis. Com muitas variáveis a tabela duplica a cada variável nova ($2^n$ linhas para $n$ variáveis), e aí passam a interessar as regras de prova da próxima página.
...
Por exemplo, considera o mundo com dois objetos: $a$ é um cubo pequeno e $b$ é um tetraedro grande. A frase $Cubo(a) \land \lnot Cubo(b)$ é verdadeira neste mundo, porque $Cubo(a)$ é V e $Cubo(b)$ é F. Mas $\forall x\, Cubo(x)$ seria falsa, porque $b$ não é cubo.

<Typst
  source={tarski}
  format="svg"
  alt="Grelha de duas casas com um quadrado pequeno a em cima à esquerda e um triângulo grande b em baixo à direita."
/>

Este jogo de "a frase é verdadeira neste mundo?" é o treino para a noção de consequência lógica: $Q$ é **consequência lógica** de $P$ quando todos os modelos de $P$ são modelos de $Q$, isto é, quando é impossível $P$ ser verdadeiro e $Q$ falso.
...
<YouTube id="itrXYg41-V0" title="Introduction to propositional logic" />

[^condicional]: O condicional material e o princípio do terceiro excluído vêm da lógica clássica usada na cadeira. As ferramentas Tarski, Fitch e Boole que os testes usam pertencem ao pacote Language, Proof and Logic, cuja página oficial descreve cada programa: [Grade Grinder](https://www.gradegrinder.net/Products/lpl-index.html).
