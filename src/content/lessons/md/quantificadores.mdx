...
import ordemQuant from './ordem-quant.typ?raw';
...
- $\lnot \exists x\, P(x) \equiv \forall x\, \lnot P(x)$. "Não há erros no programa" equivale a "tudo está sem erros".[^negacao]
...
A segunda implica a primeira, mas não o contrário. Um contraexemplo: duas pessoas $a$ e $b$ onde cada uma gosta só de si própria. Aí $\forall x\, \exists y\, Gosta(x, y)$ é verdadeira, mas $\exists y\, \forall x\, Gosta(x, y)$ é falsa, porque ninguém é gostado por ambas.

<Typst
  source={ordemQuant}
  format="svg"
  alt="Dois esquemas: em para todo x existe y cada x aponta para o seu y; em existe y para todo x o mesmo y recebe setas de todos."
/>

Testa o contraexemplo no modelo pequeno, que imprime verdadeira para a primeira ordem e falsa para a segunda:

<CodePlayground
  language="python"
  code={
    'gosta = {("a", "a"), ("b", "b")}\npessoas = ("a", "b")\nprint(all(any((x, y) in gosta for y in pessoas) for x in pessoas))\nprint(any(all((x, y) in gosta for x in pessoas) for y in pessoas))'
  }
/>

Quantificadores iguais podem trocar ($\forall x\, \forall y$ é o mesmo que $\forall y\, \forall x$); quantificadores diferentes, não.
...
[^negacao]: As leis de passagem da negação através dos quantificadores e o âmbito de cada variável seguem a entrada sobre quantificação da Stanford Encyclopedia of Philosophy: [Quantification](https://plato.stanford.edu/ENTRIES/quantification/).
