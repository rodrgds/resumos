...
import arvore from '../../me/arvore-bayes.typ?raw';
...
A árvore com os 10000 utentes hipotéticos mostra de onde vem o 1 em 6: segue os dois ramos que terminam em positivo e compara-os.

<Typst
  source={arvore}
  format="svg"
  alt="Árvore com 10000 utentes na raiz. O ramo dos 100 doentes divide-se em 99 positivos e 1 negativo; o ramo dos 9900 saudáveis divide-se em 495 positivos e 9405 negativos. Dos 594 positivos totais, só 99 estão doentes."
/>
...
<CodePlayground
  language="python"
  title="Bayes do teste de diagnóstico"
  code={
    'sens, fpos, prev = 0.99, 0.05, 0.01\np_pos = sens * prev + fpos * (1 - prev)\np_doente_dado_pos = sens * prev / p_pos\nprint("P(+):", round(p_pos, 4))\nprint("P(D|+):", round(p_doente_dado_pos, 4))'
  }
/>

Há duas formas de chegar ao mesmo 1/6. Escolhe a que te sair melhor no exame:

<Tabs id="bayes-duas-vias" labels={['Pela fórmula', 'Pela tabela de frequências']}>
  <div slot="panel-0">

Calcula o denominador pela Probabilidade Total e divide: $P(+) = 0{,}0099 + 0{,}0495 = 0{,}0594$, depois $P(D \mid +) = 0{,}0099 / 0{,}0594 = 1/6$. É direta quando decoraste a fórmula, mas cada parcela é abstrata.

  </div>
  <div slot="panel-1">

Imagina 10000 utentes: 100 doentes geram 99 positivos, 9900 saudáveis geram 495 positivos. Positivos no total: 594, dos quais 99 doentes. Logo $P(D \mid +) = 99/594 = 1/6$. Nenhuma fórmula, só contagens, e cada número tem um significado concreto.
...
Estes dois vídeos mostram a mesma inversão com intuição geométrica. Vê o primeiro para fixar a ideia de atualizar crenças, o segundo para rever o paradoxo do teste de diagnóstico.

<YouTube
  id="HZGCoVF3YvM"
  title="Teorema de Bayes, a geometria de mudar de ideias, 3Blue1Brown"
/>

<YouTube
  id="lG4VkPoG3ko"
  title="O paradoxo do teste de diagnóstico, 3Blue1Brown"
/>

## Para saber mais

- [Seeing Theory](https://seeing-theory.brown.edu/): introdução visual à probabilidade com simulações interativas, do básico às distribuições.
- [O artigo do vídeo de Bayes](https://www.3blue1brown.com/lessons/bayes-theorem/): a versão escrita da visualização geométrica, boa para rever sem carregar vídeo.
