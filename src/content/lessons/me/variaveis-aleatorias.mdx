...
import uniforme from '../../me/uniforme.typ?raw';
...
A área a sombreado é o retângulo entre 3 e 5 sob a densidade constante:

<Typst
  source={uniforme}
  format="svg"
  alt="Densidade uniforme entre 2 e 8 com altura 1/6. A faixa entre 3 e 5 está sombreada e vale 1/3 da área total."
/>

A integração numérica confirma a área sem primitiva:

<CodePlayground
  language="python"
  title="Área por soma de Riemann"
  code={
    'a, b, n = 3.0, 5.0, 10000\ndx = (b - a) / n\narea = sum((1 / 6) * dx for _ in range(n))\nprint("area:", round(area, 4))'
  }
/>

A saída dá 0,3333, o $1/3$ da conta exata. A soma de Riemann é a definição de integral a trabalhar: fatias finas vezes altura constante.
