...
import slicing from '../../p/slicing.typ?raw';
...
<Dot
  source={
    'digraph Formas { rankdir=TB; Forma [shape=box, label="Forma (abstrata)\\narea() = 0"]; Quadrado [shape=box, label="Quadrado\\narea() override"]; Circulo [shape=box, label="Circulo\\narea() override"]; Forma -> Quadrado; Forma -> Circulo; }'
  }
  alt="Hierarquia de classes: Forma abstrata com o método virtual puro area, da qual derivam Quadrado e Circulo, cada uma com a sua implementação."
/>
...
<Typst
  source={slicing}
  format="svg"
  alt="A cópia por valor guarda só a parte Base e corta o resto; a referência aponta ao objeto Derivada completo e vê tudo."
/>
...
Vamos juntar classe abstrata, duas derivadas e uma função que trata ambas pela interface comum. A área do círculo é $pi r^2$; usamos $3.1416$ como valor de $pi$. Corre o programa:

<CodePlayground
  language="cpp"
  title="Áreas por polimorfismo"
  code={
