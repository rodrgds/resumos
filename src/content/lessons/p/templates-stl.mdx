...
import intervalos from '../../p/intervalos.typ?raw';
...
<YouTube id="Jh2urtP00Zg" title="C++ Tutorial: Using the STL std::vector" />
...
## map e set: chaves e conjuntos

Nem tudo é uma sequência. O `map` associa chaves a valores, como um dicionário de Python: serve para contar ocorrências ou guardar registos por nome. O `set` guarda valores únicos e ordenados, e diz em tempo logarítmico se um valor lá está:
...
int main() {
    std::map<std::string, int> notas = {{"Ana", 15}, {"Rui", 12}};
    notas["Iris"] = 18;
    std::cout << notas["Ana"] << " " << notas.size() << "\n";
    std::set<int> vistos = {3, 1, 3, 2, 1};
    std::cout << vistos.size() << ":";
    for (int x : vistos) {
        std::cout << " " << x;
    }
    std::cout << "\n";
...
Isto escreve `15 3` e depois `3: 1 2 3`. O `map` tem três entradas depois da inserção e `notas["Ana"]` vale `15`. O `set` ignorou os repetidos e guardou `1 2 3` por ordem, por isso o tamanho é `3`. A regra de ouro: sequência ordenada ou acesso por posição pede `vector`; procura por chave pede `map`; pertença sem repetições pede `set`.
...
<Typst
  source={intervalos}
  format="svg"
  alt="Vetor com os elementos 10, 20 e 30. begin aponta ao 10 e end aponta à posição a seguir ao 30."
/>

Há duas formas de percorrer, e convém reconheceres ambas porque os exercícios misturam-nas:

<Tabs id="percorrer-vetor" labels={['Ciclo por intervalo', 'Iteradores explícitos']}>
  <div slot="panel-0">
...
A forma curta para ler todos os elementos. Esconde os iteradores, mas é sobre eles que o ciclo corre por baixo.
...
A forma explícita: `*it` lê o elemento e `it++` avança. Precisas dela quando o algoritmo pede iteradores, como o `sort` e o `find` abaixo.

  </div>
</Tabs>

Na prática escreves `auto it = v.begin()`, que pede ao compilador para deduzir o tipo comprido (`std::vector<int>::iterator`) por ti.
...
Para somar (ou juntar) todos os elementos com um valor inicial existe o `accumulate` de `<numeric>`:
...
Isto escreve `10`. O terceiro argumento é o valor inicial e o tipo do resultado: com `0` somas inteiros, com `0.0` somavas reais.

## Exemplo completo: contar e ordenar

Vamos escrever uma função genérica `contar_maiores` que conta quantos elementos de um vetor excedem um limite, e usá-la com dois tipos diferentes. Depois ordenamos um dos vetores com `sort`. Corre o programa:

<CodePlayground
  language="cpp"
  title="Contar e ordenar com templates e STL"
  code={
...
- [Referência de `std::vector`](https://en.cppreference.com/cpp/container/vector): métodos, complexidade e exemplos na documentação oficial.
