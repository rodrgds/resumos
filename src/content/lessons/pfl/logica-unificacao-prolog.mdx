...
import emparelha from '../../pfl/p7unifica.typ?raw';
import arvoreSLD from '../../pfl/p7sld.dot?raw';
...
O diagrama emparelha os dois primeiros exemplos com os factos: à esquerda a query, à direita o facto, e no meio a substituição que os iguala. O terceiro par não tem seta de sucesso porque nenhum facto casa `tomas` com `ana` na segunda posição.

<Typst
  source={emparelha}
  format="svg"
  alt="Unificação de pai(ana, X) com pai(ana, leo) pela substituição X igual a leo; pai(X, leo) com pai(tomas, ana) falha."
/>
...
A árvore completa da query, com o ramo de sucesso e o beco do retrocesso:

<Dot
  source={arvoreSLD}
  alt="Árvore SLD de avo(ana, X): a query unifica com a regra, pai(ana, Y) casa com o facto dando Y igual a leo, pai(leo, X) casa dando X igual a rui; o retrocesso não encontra alternativas."
/>

Corre a query no navegador com os factos da página:

<CodePlayground
  language="prolog"
  title="avô da Ana no navegador"
  code={
    'pai(tomas, ana).\npai(ana, leo).\npai(leo, rui).\navo(X, Z) :- pai(X, Y), pai(Y, Z).\n\nmain :- avo(ana, X), writeln(X).'
  }
/>

Carrega em **Executar**: imprime `rui`, a mesma resposta construída à mão na árvore.
...
<YouTube id="ycHLj12bm0M" title="Começar em Prolog (curso em português)" />

- [SWISH](https://swish.swi-prolog.org/): corre Prolog no navegador, com tutoriais embutidos para experimentar as queries destas páginas.
- [Learn Prolog Now, capítulo 2](https://lpn.swi-prolog.org/lpnpage.php?pageid=lpn-htmlch2&pagetype=html): unificação passo a passo, com exercícios.
- [Unificação no Wikibooks](https://pt.wikibooks.org/wiki/Prolog/Unifica%C3%A7%C3%A3o): a mesma matéria explicada em português, com exemplos.
