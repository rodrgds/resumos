...
import hierarquia from '../../pfl/p2classes.typ?raw';
...
Confirma no navegador, com números e com carateres:

<CodePlayground
  language="haskell"
  title="contem com Eq"
  code={
    'contem :: Eq a => a -> [a] -> Bool\ncontem x xs = x `elem` xs\n\nmain :: IO ()\nmain = do\n  print (contem 3 [1,2,3])\n  print (contem \'a\' "ola")'
  }
/>

Carrega em **Executar**: imprime `True` duas vezes.
...
Repara que `Ord` implica `Eq` e `Num` implica `Eq`: para ordenar é preciso comparar igualdade, e a hierarquia reflete isso. O diagrama resume as dependências: cada seta lê-se "exige".

<Typst
  source={hierarquia}
  format="svg"
  alt="Hierarquia de classes: Eq no topo; Ord e Num dependem de Eq; Integral e Fractional dependem de Num."
/>

Quando vires uma assinatura com várias restrições, como `(Ord a, Show a) => ...`, lê como uma lista de capacidades exigidas.
...
<Tabs id="erro-num" labels={['O erro', 'A cura']}>
  <div slot="panel-0">

```text
ghci> length [1,2] + 0.5
-- erro: nenhum membro Fractional para Int.
-- O + quer os dois lados do mesmo tipo.
```
...
```text
ghci> fromIntegral (length [1,2]) + 0.5
2.5
```
...
- [Hoogle](https://hoogle.haskell.org/): pesquisa funções de Haskell por nome ou por assinatura de tipo, como `Eq a => a -> [a] -> Bool`.
  :::
