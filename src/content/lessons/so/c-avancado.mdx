...
import fasesGcc from './fases-gcc.typ?raw';
...
O `gcc` não compila de uma vez: **pré-processa** (expande `#include` e macros), **compila** (traduz C para assembly), **monta** (gera código objeto) e **liga** (junta o teu código objeto com as bibliotecas num executável).

<Typst
  source={fasesGcc}
  format="svg"
  alt="As cinco fases do gcc em cadeia: ficheiro fonte, pré-processador, compilador, montador, ligador e executável."
/>

Podes ver as fases com `gcc -save-temps`. O erro que o compilador te dá pertence sempre a uma fase: um `#include` errado falha no pré-processamento, um `;` em falta falha na compilação, uma função declarada mas nunca definida falha na ligação com `undefined reference`.
...
<Tabs id="opcoes-gcc" labels={['Avisos', 'Ver as fases', 'Depurar']}>
  <div slot="panel-0">
...
Liga os avisos: apanha variáveis por usar, comparações suspeitas e formatos errados no `printf` antes de correres.

  </div>
  <div slot="panel-1">

```sh
gcc -save-temps programa.c -o programa
ls programa.*
```

Guarda os ficheiros intermédios (`.i` pré-processado, `.s` em assembly, `.o` objeto) para veres cada fase do diagrama.
...
Inclui símbolos de depuração para poderes parar em linhas e inspecionar variáveis no `gdb`.
...
Cada processo vê três zonas principais: o **código** e os dados fixos, a **pilha** (_stack_), que cresce e encolhe com as chamadas de função, e a **área livre** (_heap_), onde o `malloc` reserva blocos do tamanho que pedires e o `free` os devolve. A pilha limpa-se sozinha quando a função retorna; a área livre só se limpa se tu a libertares. Confirma os tamanhos base no teu ambiente:

<CodePlayground
  language="c"
  title="Tamanhos em C"
  code={'#include <stdio.h>\n\nint main(void) {\n    printf("int: %zu, ponteiro: %zu\\n", sizeof(int), sizeof(void *));\n    return 0;\n}'}
/>

Isto imprime `int: 4, ponteiro: 8` num Linux de 64 bits. O `sizeof` conta bytes em tempo de compilação: é com ele que se calcula o argumento do `malloc`, como `3 * sizeof(int)` mais abaixo.
