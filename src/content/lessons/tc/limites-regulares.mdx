...
import caminhoBombeamento from '../../tc/caminho-bombeamento.typ?raw';
...
Exemplo resolvido. Seja $L = \{w \mid w \text{ tem igual n\'umero de } 0 \text{ e } 1\}$. Supõe que $L$ é regular. A linguagem $R = 0^*1^*$ é regular (é dada por uma expressão regular). Pelo fecho para interseção, $L \cap R$ seria regular. Mas $L \cap R = \{0^n 1^n \mid n \ge 0\}$: uma palavra com igual número de zeros e uns que tenha a forma $0^*1^*$ tem necessariamente todos os zeros antes dos uns, logo é $0^n1^n$. Ora $\{0^n 1^n\}$ não é regular, pela prova acima. Contradição, por isso $L$ não é regular. Repara no passo delicado: $L \cap R$ não é "mais ou menos" $0^n1^n$, é exatamente; sem essa igualdade a prova cai.
